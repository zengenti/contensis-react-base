...
const selectedHistory =
  typeof window !== 'undefined'
    ? history$1.createBrowserHistory
    : history$1.createMemoryHistory;
...
const navigationSagas = [
  effects.takeEvery(navigation.GET_NODE_TREE, ensureNodeTreeSaga),
];
function* ensureNodeTreeSaga(action) {
  ...
  try {
    if (!navigation.hasNavigationTree(state)) {
      const deliveryApiVersionStatus = yield effects.select(
        version.selectVersionStatus
      );
...
const fieldExpression = (field, value, operator = 'equalTo', weight = null) => {
  ...
  if (Array.isArray(value)) return equalToOrIn(field, value, operator);
    ...
    return !weight
      ? [contensisDeliveryApi.Op[operator](field, value)]
      : [contensisDeliveryApi.Op[operator](field, value).weight(weight)];
};
const defaultExpressions = versionStatus => {
  return [
    contensisDeliveryApi.Op.equalTo(Fields.sys.versionStatus, versionStatus),
  ];
};
...
const equalToOrIn = (field, arr, operator = 'equalTo') =>
  arr.length === 0
    ? []
    : arr.length === 1
    ? [contensisDeliveryApi.Op[operator](field, arr[0])]
    : [contensisDeliveryApi.Op.in(field, ...arr)];

// eslint-disable-next-line import/named
const routeEntryByFieldsQuery = (
  id,
  language = 'en-GB',
  fields = [],
  versionStatus = 'published'
) => {
  const query = new contensisDeliveryApi.Query(
    ...[
...
// load-entries.js
const routingSagas = [
  effects.takeEvery(routing.SET_NAVIGATION_PATH, getRouteSaga),
  effects.takeEvery(routing.SET_ROUTE, setRouteSaga),
];
/**
...
function* getRouteSaga(action) {
  ...
  try {
    ...
    if (
      ...
      if (
        ...
        yield effects.call(
          ...
          yield effects.select(routing$1.selectCurrentNode),
          yield effects.select(routing$1.selectCurrentAncestors)
        );
    } else {
      // Handle homepage
      if (isHome) {
        ...
        if (isPreview) {
          ...
          if (
            ...
            const { fields, linkDepth, nodeOptions = {} } =
              routing.findContentTypeMapping(
                ContentTypeMappings,
    ...
    if (
      ...
      if (pathNode)
        yield effects.call(setRouteEntry, null, pathNode, ancestors, siblings);
      else if (!staticRoute) yield effects.call(do404);
    }
    ...
    if (
      !navigation.hasNavigationTree(state) &&
...
function* setRouteEntry(
  ...
  const currentEntryId = yield effects.select(
    routing$1.selectRouteEntryEntryId
  );
  const currentEntryLang = yield effects.select(
    routing$1.selectRouteEntryLanguage
  );
  const mappedEntry =
    currentEntryId === entrySys.id && currentEntryLang === entrySys.language
      ? (
          (yield effects.select(routing$1.selectMappedEntry)) || immutable.Map()
        ).toJS()
...
const registerSagas = [
  effects.takeEvery(reducers.REGISTER_USER, registerSaga),
  effects.takeEvery(reducers.REGISTER_USER_SUCCESS, redirectSaga),
];
...
function* redirectSaga() {
  ...
  const currentQs = routing$1.queryParams(
    yield effects.select(routing$1.selectCurrentSearch)
  );
...
class UserHelper {
  ...
  static async RequestPasswordReset(userEmailObject) {
    const options = { ...BASE_OPTIONS, body: JSON.stringify(userEmailObject) };
    options.method = 'POST';
...
const resetPasswordSagas = [
  effects.takeEvery(
    reducers.REQUEST_USER_PASSWORD_RESET,
    requestPasswordResetSaga
  ),
  effects.takeEvery(reducers.RESET_USER_PASSWORD, resetPasswordSaga),
  effects.takeEvery(reducers.CHANGE_USER_PASSWORD, changePasswordSaga),
];
...
//# sourceMappingURL=App-a3a3c76c.js.map
