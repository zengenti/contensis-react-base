...
var mapJson = require('jsonpath-mapper');
>>>>>>> contensis-14-forgot-change-password:cjs/login-93c36858.js
...
const getManagementApiClient = async ({
  ...
  const { Client } = await Promise.resolve().then(function() {
    return /*#__PURE__*/ _interopNamespace(require('contensis-management-api'));
  });
...
var mapClientCredentials = obj =>
  mapJson__default['default'](obj, clientCredentials);
...
>>>>>>> contensis-14-forgot-change-password:cjs/login-93c36858.js
  ...
  static async LoginUser({ username, password, clientCredentials }) {
    ...
    if (username && password) {
      ...
      const [loginError, clientBearerToken] = await awaitToJs.to(
  ...
  static async GetCredentialsForSecurityToken(securityToken) {
    const [error, response] = await awaitToJs.to(
...
LoginHelper.GetUserDetails = async client => {
  ...
  if (user && user.id) {
    [groupsError, groupsResult] = await awaitToJs.to(
...
const loginSagas = [
  effects.takeEvery(reducers.LOGIN_USER, loginUserSaga),
  effects.takeEvery(reducers.LOGOUT_USER, logoutUserSaga),
  effects.takeEvery(reducers.VALIDATE_USER, validateUserSaga),
  effects.takeEvery(
    reducers.SET_AUTHENTICATION_STATE,
...
=======
  const currentQs = routing.queryParams(
    yield effects.select(routing.selectCurrentSearch)
  );
  const securityToken = currentQs.securityToken || currentQs.securitytoken; // Check if any of the defined routes have "requireLogin" attribute
  ...
  const { requireLogin: authContentType } =
    (entry &&
      routing$1.findContentTypeMapping(
        ContentTypeMappings,
...
function* loginUserSaga(action = {}) {
  ...
  if (action.type === reducers.LOGIN_USER && LoginHelper.WSFED_LOGIN)
...
function* refreshSecurityToken() {
  const clientCredentials = (
    (yield effects.select(ToJs.selectClientCredentials)) || immutable.Map()
  ).toJS();
...
exports.loginSagas = loginSagas;
exports.refreshSecurityToken = refreshSecurityToken;
//# sourceMappingURL=login-93c36858.js.map
>>>>>>> contensis-14-forgot-change-password:cjs/login-93c36858.js
