...
const getFacets = state => state.getIn(['search', Context.facets], immutable.Map());
...
const getTabsAndFacets = state => {
  ...
  return (tabs || immutable.List()).map(tab => {
...
const fieldExpression = (field, value, operator = 'equalTo', weight = null) => {
  ...
  if (Array.isArray(field)) // If an array of fieldIds have been provided, call self for each fieldId
    ...
    return [contensisDeliveryApi.Op.or(...field.map(fieldId => fieldExpression(fieldId, value, operator, weight)).flat())];
