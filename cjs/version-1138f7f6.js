...
var createSagaMiddleware__default = /*#__PURE__*/ _interopDefaultLegacy(
...
var NavigationReducer = (state = initialState, action) => {
  switch (action.type) {
    case SET_NODE_TREE: {
      return state
        .set('root', immutable.fromJS(action.nodes))
        .set('isReady', true);
    }
...
var RoutingReducer = (state = initialState$1, action) => {
  switch (action.type) {
    case routing.SET_ANCESTORS: {
      if (action.ancestors) {
        return state.set(
          ...
          immutable.fromJS(action.ancestors)
        );
      }
    ...
    case routing.SET_ENTRY: {
      ...
      if (!entry) {
        ...
        nextState = state
          ...
          .set('entry', immutable.fromJS(entry))
          .set('error', immutable.fromJS(error))
          .set('isError', isError)
          .set('isLoading', isLoading)
          .set('notFound', notFound)
        ...
        if (mappedEntry && Object.keys(mappedEntry).length > 0)
          nextState = nextState
            .set('mappedEntry', immutable.fromJS(mappedEntry))
            .set(
              'entry',
              immutable.fromJS({
                sys: entry.sys,
      ...
      if (!node) {
        ...
        return nextState
          .set('currentNode', immutable.fromJS(node))
    ...
    case routing.SET_NAVIGATION_PATH: {
      ...
      if (action.path) {
        ...
        if (entryUri != action.path) {
          return state
            .set('currentPath', immutable.fromJS(action.path))
    ...
    case routing.SET_SIBLINGS: {
      ...
      const allNodeDepends = immutable.Set.union([
        immutable.Set(siblingIDs),
        currentNodeDepends,
      ]);
      return state
        .set('nodeDepends', allNodeDepends)
        .set('currentNodeSiblings', immutable.fromJS(action.siblings))
...
const setVersion = (commitRef, buildNo) =>
  routing.action(SET_VERSION, {
    commitRef,
    buildNo,
  });
const setVersionStatus = status =>
  routing.action(SET_VERSION_STATUS, {
    status,
  });
...
//# sourceMappingURL=navigation-8c90cb78.js.map
