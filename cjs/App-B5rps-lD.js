...
var store = require('./store-2Rx8ZF9-.js');
var version = require('./version-nvHl-RQB.js');
var to = require('await-to-js');
var createSagaMiddleware = require('redux-saga');
var contensisDeliveryApi = require('contensis-delivery-api');
var slice = require('./slice-5xJMH24n.js');
var selectors$1 = require('./selectors-PS7proOd.js');
var version$1 = require('./version-DBVUgqWk.js');
var ContensisDeliveryApi = require('./ContensisDeliveryApi-BTooAJDI.js');
var React = require('react');
require('react-cookie');
require('react-redux');
require('./VersionInfo-BVyI7gmw.js');
require('react-helmet-async');
require('jsonpath-mapper');
require('react-router-dom');
var selectors = require('./selectors-DAQR0uZa.js');
var ChangePassword_container = require('./ChangePassword.container-CTos0DYb.js');
require('./sagas-DOJJCnqw.js');
require('reselect');
require('immer');
require('deep-equal');
require('deepmerge');
var util = require('./util-B_Ato2qm.js');
require('contensis-core-api');
var selectors$2 = require('./selectors-BrHyPW7a.js');
var RouteLoader = require('./RouteLoader-CyxOHDDk.js');
...
const selectedHistory = typeof window !== 'undefined' ? history$2.createBrowserHistory : history$2.createMemoryHistory;
...
const releaseAccessToken = async releaseUri => {
  const [error, response] = await to__default.default(fetch(releaseUri, {
    method: 'POST',
    credentials: 'same-origin'
  }));
  if (error || !response.ok) return [error || new Error(`Release failed: ${response.status}`)];
  const [parseError, body] = await to__default.default(response.json());
...
function* ensureAccessTokenSaga(action) {
  ...
  if (ContensisDeliveryApi.isSSR || ((_action$location = action.location) === null || _action$location === void 0 ? void 0 : _action$location.pathname) === ChangePassword_container.LoginHelper.ACCESS_DENIED_ROUTE) return true;
  ...
  if (config.releaseUri) {
    var _action$ssr;
    const [releaseError, accessToken] = yield effects.call(releaseAccessToken, config.releaseUri);
    ...
    if (releaseError || !accessToken || !userLoggedIn) {
      ...
      ChangePassword_container.LoginHelper.ClientRedirectToAccessDeniedPage((_action$location2 = action.location) === null || _action$location2 === void 0 ? void 0 : _action$location2.pathname);
      return false;
    }
...
function* getRouteSaga(action) {
  ...
  try {
    ...
    if (livePreviewTask) {
      yield effects.cancel(livePreviewTask);
    ...
    if (typeof window !== 'undefined' && !livePreviewNavigationTask && (parent !== window || opener)) {
      livePreviewNavigationTask = yield effects.spawn(watchLivePreviewNavigationSaga);
    }

    // Resolve an access token that can serve this route before we request
    // any content with it
    if (!(yield effects.call(ensureAccessTokenSaga, action))) {
      yield effects.put({
        ...
        if (nodeError) {
          if ([401, 403].includes(nodeError.status)) {
            ...
            if (userLoggedIn && nodeError.status === 401) {
              ...
              const accessDeniedRoute = ChangePassword_container.LoginHelper.GetAccessDeniedRoute(currentPath);
    ...
    if ((_pathNode5 = pathNode) !== null && _pathNode5 !== void 0 && (_pathNode5 = _pathNode5.entry) !== null && _pathNode5 !== void 0 && (_pathNode5 = _pathNode5.sys) !== null && _pathNode5 !== void 0 && _pathNode5.id) {
      ...
      yield effects.call(setRouteEntry, currentPath, entry, pathNode, ancestors, siblings, entryMapper, false, (_appsays5 = appsays) === null || _appsays5 === void 0 ? void 0 : _appsays5.refetchNode);
    } else {
...
function createLivePreviewChannel() {
  return createSagaMiddleware.eventChannel(emit => {
    const handler = e => {
      ...
      if ((_e$data = e.data) !== null && _e$data !== void 0 && (_e$data = _e$data.type) !== null && _e$data !== void 0 && _e$data.startsWith(selectors$1.LIVE_ENTRY_PREFIX) && e.data.type !== selectors$1.LIVE_ENTRY_NAVIGATE) emit(e.data);
    };
...
function* watchLivePreviewNavigationSaga() {
  const channel = createSagaMiddleware.eventChannel(emit => {
    const handler = e => {
      var _e$data2;
      if (((_e$data2 = e.data) === null || _e$data2 === void 0 ? void 0 : _e$data2.type) === selectors$1.LIVE_ENTRY_NAVIGATE) emit(e.data);
    };
    window.addEventListener('message', handler);
    return () => window.removeEventListener('message', handler);
  });
...
function* watchLivePreviewSaga(context) {
  ...
  const routeLoadOptions = {
    type: selectors$1.LIVE_ENTRY_ROUTE_LOAD_OPTIONS,
  ...
  try {
    while (true) {
      ...
      if (data.type === selectors$1.LIVE_ENTRY_RESET) {
        console.log('Resetting live preview entry to original route entry');
        entry = context.entry;
      } else if (data.type === selectors$1.LIVE_ENTRY_UPDATE) {
...
function* changePasswordSaga(action) {
  ...
  try {
    ...
    const clientCredentials = yield effects.select(selectors$2.selectClientCredentials, 'js');
...
//# sourceMappingURL=App-B5rps-lD.js.map
