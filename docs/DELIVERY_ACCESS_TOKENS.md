...
- **No `requireLogin`** — the token is injected into the SSR response as usual. Pages render server-side as normal.
...
- **`requireLogin: [{ id: 'group-id' }]`** — as above, but the token is only released to members of at least one of the listed user groups.
