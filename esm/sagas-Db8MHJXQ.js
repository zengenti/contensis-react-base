...
import { L as LruCache, g as getClientConfig, c as cachedSearch } from './ContensisDeliveryApi-Cv7MK7e6.js';
import { Client } from 'contensis-delivery-api';
import { r as removeEmptyAttributes, e as extractQuotedPhrases, f as fixFreeTextForElastic, d as convertKeyForAggregation, g as getItemsFromResult, b as cleanseFieldIdForAggregation, a as areArraysEqualSets, c as callCustomApi, t as timedSearch } from './util-BqG6FtNI.js';
...
import { a6 as selectStaticRoute, N as selectCurrentPath, O as selectCurrentProject } from './selectors-DIKsu8SE.js';
...
const getFacet$1 = (state, facetName = '', context = Context.facets, returnType) => {
...
const getIsLoading$2 = (state, context = Context.facets, facet) => {
...
const getFeaturedResults$2 = (state, current = '', context = Context.facets, returnType) => {
...
const getPageIndex$2 = (state, current = '', context = Context.facets) => {
...
const getPageSize = (state, current = '', context = Context.facets) => {
  return getImmutableOrJS(state, ['search', context, current || getCurrent$2(state, context), 'pagingInfo', 'pageSize'], 0 // Defaults to 0 because we want it to fall back to a query param if not defined
  );
};
...
const getPageIsLoading$2 = (state, current = '', context = Context.facets) => {
...
const getTotalCount$1 = (state, current = '', context = Context.facets) => {
...
const selectListing = {
  ...
  getFeaturedResults: (state, listing = '') => getFeaturedResults$2(state, listing, Context.listings, 'js'),
  ...
  getPageIsLoading: (state, listing = '') => getPageIsLoading$2(state, listing, Context.listings),
  ...
  getRenderableFilters: (state, listing = '') => getRenderableFilters$2(state, listing, Context.listings),
...
const useFacets = ({
  ...
  const dispatchProps = {
    ...
    updatePageIndex: (pageIndex, scrollToElement) => dispatch(withMappers(updatePageIndex$1(pageIndex, scrollToElement), m)),
    updatePageSize: (pageSize, scrollToElement) => dispatch(withMappers(updatePageSize$1(pageSize, scrollToElement), m)),
...
const fieldExpression = (field, value, operator = 'equalTo', weight, fuzzySearch) => {
...
const equalToOrIn = (field, value, operator = 'equalTo', fuzzySearch) => {
...
const termExpressions = (searchTerm, weightedSearchFields, fuzzySearch, omitSearchFields = []) => {
  if (searchTerm && weightedSearchFields && weightedSearchFields.length > 0) {
    ...
    const freeTextOp = (f, term) => {
      var _f$fuzzySearch;
      return fieldExpression(f.fieldId, fixFreeTextForElastic(term), 'freeText', f.weight, (_f$fuzzySearch = f.fuzzySearch) != null ? _f$fuzzySearch : fuzzySearch);
    };
...
const queryParamsTemplate = {
  aggregations: root => {
    ...
    return {
      ...aggregations,
      ...getQueryParameter$2(root, 'customAggregations', {})
    };
  },
  ...
  fieldLinkDepths: root => getQueryParameter$2(root, 'fieldLinkDepths', []),
  ...
  fuzzySearch: root => getQueryParameter$2(root, 'fuzzySearch', false),
  ...
  pageSize: root => {
    ...
    return getPageSize(state, action.facet, action.context) || getQueryParameter$2(root, 'pageSize');
  },
  pagesLoaded: ({
    state,
    facet,
...
//# sourceMappingURL=sagas-Db8MHJXQ.js.map
