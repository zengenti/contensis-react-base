...
import { takeEvery, select, put, call, cancel, spawn, fork, take, cancelled, all } from '@redux-saga/core/effects';
import * as log from 'loglevel';
import { error as error$1 } from 'loglevel';
import { G as GET_NODE_TREE, S as SET_NODE_TREE, a as GET_NODE_TREE_ERROR } from './store-BVx96VK4.js';
import { h as hasNavigationTree, a as injectRedux } from './version-DeSpihKR.js';
...
import { N as selectCurrentPath, ab as setRoute, Y as selectRouteEntryAvailableLanguages, $ as selectRouteEntryID, a6 as selectStaticRoute, w as SET_NAVIGATION_PATH, x as SET_ROUTE, U as UPDATE_LOADING_STATE, X as selectRouteEntry, M as selectCurrentNode, O as selectCurrentProject, K as selectCurrentAncestors, Q as selectCurrentSiblings, f as LIVE_ENTRY_ROUTE_LOAD_OPTIONS, e as LIVE_ENTRY_RESET, g as LIVE_ENTRY_UPDATE, _ as selectRouteEntryEntryId, a0 as selectRouteEntryLanguage, W as selectMappedEntry, v as SET_ENTRY, S as SET_ANCESTORS, y as SET_SIBLINGS, L as LIVE_ENTRY_NAVIGATE, d as LIVE_ENTRY_PREFIX, R as REGISTER_USER, k as REGISTER_USER_SUCCESS, j as REGISTER_USER_FAILED, G as queryParams, P as selectCurrentSearch, l as REQUEST_USER_PASSWORD_RESET, p as RESET_USER_PASSWORD, C as CHANGE_USER_PASSWORD, n as REQUEST_USER_PASSWORD_RESET_SENDING, o as REQUEST_USER_PASSWORD_RESET_SUCCESS, m as REQUEST_USER_PASSWORD_RESET_ERROR, r as RESET_USER_PASSWORD_SENDING, s as RESET_USER_PASSWORD_SUCCESS, q as RESET_USER_PASSWORD_ERROR, a as CHANGE_USER_PASSWORD_ERROR, b as CHANGE_USER_PASSWORD_SENDING, c as CHANGE_USER_PASSWORD_SUCCESS } from './selectors-DIKsu8SE.js';
import { b as selectVersionStatus } from './version-7tjXt-WE.js';
import { c as cachedSearch, i as isSSR } from './ContensisDeliveryApi-Cv7MK7e6.js';
import React from 'react';
import 'react-cookie';
import 'react-redux';
import './VersionInfo-BKoTP8ct.js';
import 'react-helmet-async';
import 'jsonpath-mapper';
import 'react-router-dom';
import { s as selectCurrentLanguage, a as selectDictionary, c as selectLocaleRoutes, d as selectLocales, b as selectDictionaryResolver, e as selectPrimaryLanguage } from './selectors-D4ulaTom.js';
import { f as findContentTypeMapping, h as handleRequiresLoginSaga, L as LoginHelper, c as getSearchOptions, g as getManagementApiClient, l as loginSagas } from './ChangePassword.container-Cd4fHjCN.js';
import './sagas-Db8MHJXQ.js';
import 'reselect';
import 'immer';
import 'deep-equal';
import 'deepmerge';
import { h as routeParams } from './util-BqG6FtNI.js';
import 'contensis-core-api';
import { c as selectClientCredentials } from './selectors-Dtb-Npd7.js';
import { a as RouteLoader } from './RouteLoader-DYEFOZos.js';
import 'query-string';
...
const history$1 = (options = {}) => selectedHistory(options);
...
const releaseAccessToken = async releaseUri => {
  const [error, response] = await to(fetch(releaseUri, {
    method: 'POST',
    credentials: 'same-origin'
  }));
  if (error || !response.ok) return [error || new Error(`Release failed: ${response.status}`)];
...
function* ensureAccessTokenSaga(action) {
  ...
  if (isSSR || ((_action$location = action.location) === null || _action$location === void 0 ? void 0 : _action$location.pathname) === LoginHelper.ACCESS_DENIED_ROUTE) return true;
  ...
  if (config.releaseUri) {
    var _action$ssr;
    const [releaseError, accessToken] = yield call(releaseAccessToken, config.releaseUri);
    ...
    if (releaseError || !accessToken || !userLoggedIn) {
      var _action$location2;
      error(releaseError, 'unable to release an access token');
      // Authenticated but nothing released
      LoginHelper.ClientRedirectToAccessDeniedPage((_action$location2 = action.location) === null || _action$location2 === void 0 ? void 0 : _action$location2.pathname);
      return false;
    }
    ...
    ((_action$ssr = action.ssr) === null || _action$ssr === void 0 ? void 0 : _action$ssr.config) && (action.ssr.config.accessToken = accessToken);
  }
  return true;
}
function* getRouteSaga(action) {
  ...
  try {
    ...
    if (typeof window !== 'undefined' && !livePreviewNavigationTask && (parent !== window || opener)) {
    ...
    if (!isPreview && ((_appsays3 = appsays) !== null && _appsays3 !== void 0 && _appsays3.customRouting || staticRoute && !staticRoute.route.fetchNode || routeEntry && action.statePath === action.path && ((_appsays4 = appsays) === null || _appsays4 === void 0 ? void 0 : _appsays4.refetchNode) !== true)) {
      ...
      if (routeEntry && (!staticRoute || staticRoute !== null && staticRoute !== void 0 && (_staticRoute$route5 = staticRoute.route) !== null && _staticRoute$route5 !== void 0 && _staticRoute$route5.fetchNode)) {
        ...
        if (params.livePreview && typeof window !== 'undefined') {
          ...
          fields = setStaticRouteLimits ? fields || '*' : ((_contentTypeMapping = contentTypeMapping) === null || _contentTypeMapping === void 0 ? void 0 : _contentTypeMapping.fields) || '*';
          linkDepth = setStaticRouteLimits ? linkDepth || 0 : typeof ((_contentTypeMapping2 = contentTypeMapping) === null || _contentTypeMapping2 === void 0 ? void 0 : _contentTypeMapping2.linkDepth) !== 'undefined' ? contentTypeMapping.linkDepth : defaultLinkDepth;
        ...
        if (nodeError) {
          if ([401, 403].includes(nodeError.status)) {
            ...
            if (userLoggedIn && nodeError.status === 401) {
              ...
              const accessDeniedRoute = LoginHelper.GetAccessDeniedRoute(currentPath);
...
function createLivePreviewChannel() {
  return eventChannel(emit => {
    const handler = e => {
      ...
      if ((_e$data = e.data) !== null && _e$data !== void 0 && (_e$data = _e$data.type) !== null && _e$data !== void 0 && _e$data.startsWith(LIVE_ENTRY_PREFIX) && e.data.type !== LIVE_ENTRY_NAVIGATE) emit(e.data);
    };
...
function* watchLivePreviewNavigationSaga() {
  const channel = eventChannel(emit => {
    const handler = e => {
      var _e$data2;
      if (((_e$data2 = e.data) === null || _e$data2 === void 0 ? void 0 : _e$data2.type) === LIVE_ENTRY_NAVIGATE) emit(e.data);
    };
    window.addEventListener('message', handler);
    return () => window.removeEventListener('message', handler);
  });
  try {
    while (true) {
      ...
      if (direction === 'back') history.back();else if (direction === 'forward') history.forward();
    }
  } finally {
...
function* watchLivePreviewSaga(context) {
  ...
  (_ref = parent !== window ? parent : opener) === null || _ref === void 0 || _ref.postMessage(routeLoadOptions, '*');
  try {
    while (true) {
...
export { AppRoot as A, browserHistory as b, createLocaleRoutes as c, history$1 as h, logError as l, pickProject as p, rootSaga as r, shorten as s };
//# sourceMappingURL=App-CZmUtJQ5.js.map
