...
const getImmutableOrJS = (state, stateKey, fallbackValue) => {
  if ('get' in state && typeof state.get === 'function' && 'getIn' in state && typeof state.getIn === 'function' && 'toJS' in state && typeof state.toJS === 'function') {
    if (Array.isArray(stateKey)) return state.getIn(stateKey, fallbackValue);
    return state.get(stateKey, fallbackValue);
  }
